...
class DiagnosticScoreView(View):
    ...
    async def command(self):
        ...
        from src.bot.views.practice import generate_practice_plan  # imported from new helpers
        ...
        flow_result = await diagnostics_flow.process_diagnostic_score(self.context, question_id, score)
        ...
        if flow_result.status in (FlowStatus.COMPLETED, FlowStatus.DONE):
            ...
            with get_session() as session:
                user = services.get_or_create_user(session, telegram_id=self.update.effective_user.id)
                user_progress = session.get(services.UserProgress, self.context.user_data.get('active_progress_id'))

                success, practice_result = await generate_practice_plan(self.context, session, user, user_progress)
