...
if not llm_api_key:
    logger.warning("OPENAI_API_KEY not found in environment variables! LLM features will not work. Setting llm to None.")
    llm = None
else:
    try:
        llm = ChatOpenAI(model_name="gpt-3.5-turbo", temperature=0.7, openai_api_key=llm_api_key)
...
if __name__ == '__main__':
    ...
    if llm is None: # Проверяем, был ли LLM успешно инициализирован
        logger.warning("LLM (ChatOpenAI) is not available (likely missing OPENAI_API_KEY or initialization failed). LLM-dependent features will be disabled.")
