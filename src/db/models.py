...
class User(SQLModel, table=True):
    ...
    active_language_id: Optional[int] = Field(
        default=None, foreign_key="programminglanguage.id"
    )
    created_at: datetime.datetime = Field(default_factory=datetime.datetime.utcnow)
    state: str = Field(
        default="lang_select"
    )  # Текущее состояние пользователя (для state machine)
    ...
    answers: List["UserAnswer"] = Relationship(
        back_populates="user"
    )  # All answers by user across all languages
...
class Question(SQLModel, table=True):
    ...
    author_notes: Optional[str] = Field(
        default=None
    )  # e.g., correct answer hints, internal notes
...
class UserProgress(SQLModel, table=True):
    ...
    diagnostic_scores_json: Optional[str] = Field(
        default=None
    )  # JSON string for self-assessment scores
    diagnostics_completed: bool = Field(
        default=False
    )  # Явно отмечает завершение диагностики
    created_at: datetime.datetime = Field(default_factory=datetime.datetime.utcnow)
    ...
    language: Optional[ProgrammingLanguage] = Relationship()
    learning_plan_items: List["UserLearningPlanItem"] = Relationship(
        back_populates="user_progress"
    )

    __table_args__ = (
        UniqueConstraint("user_id", "language_id", name="uq_user_language_progress"),
    )
...
class UserLearningPlanItem(SQLModel, table=True):
    ...
    status: str = Field(
        default="pending"
    )  # e.g., "pending", "current", "answered", "skipped"
    assigned_at: datetime.datetime = Field(default_factory=datetime.datetime.utcnow)

    user_progress: Optional[UserProgress] = Relationship(
        back_populates="learning_plan_items"
    )
...
class UserAnswer(SQLModel, table=True):
    ...
    user_id: int = Field(
        foreign_key="user.id"
    )  # Direct link to user for easier querying of all user's answers
    question_id: int = Field(foreign_key="question.id")
    learning_plan_item_id: int = Field(
        foreign_key="userlearningplanitem.id"
    )  # Link to the specific instance in a plan
    answer_text: str
    llm_assessment: Optional[str] = Field(
        default=None
    )  # AI's textual feedback on the answer
    llm_explanation: Optional[str] = Field(
        default=None
    )  # AI's detailed explanation of the topic
    is_correct_by_llm: Optional[bool] = Field(
        default=None
    )  # If AI can provide a boolean correct/incorrect
