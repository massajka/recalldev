...
from src.db.models import Category, ProgrammingLanguage, Question
...
INITIAL_DATA = {
    ...
    "categories": [
        ...
        },  # Добавлено, т.к. используется в вопросах JS
    ],
...
def _create_question(
    ...
    if existing_question:
        logger.warning(
            f"Question with text '{text}' for lang_id={language_id}, cat_id={category_id} already exists. Skipping."
        )
    ...
    logger.info(
        f"Prepared question for creation: {text[:50]}... for lang_id={language_id}, cat_id={category_id}"
    )
    return question
...
def populate_initial_data_hardcoded(session: Session):
    ...
    for lang_data in INITIAL_DATA["languages"]:
        ...
        if (
            lang
        ):  # Если язык был только что подготовлен (или уже существовал и был получен)
    ...
    for lang_slug, questions_data in INITIAL_DATA["diagnostic_questions"].items():
        ...
        if not language:
            logger.warning(
                f"Language slug '{lang_slug}' for diagnostic questions not found in loaded languages. Skipping questions for this language."
            )
            continue
        ...
        for q_data in questions_data:
            ...
            if not category:
                logger.warning(
                    f"Category '{q_data['category']}' for diagnostic question '{q_data['text'][:30]}...' not found in loaded categories. Skipping this question."
                )
                continue
            ...
            if temp_lang_id is None or temp_cat_id is None:
                logger.info(
                    "Flushing session to obtain IDs for newly created languages/categories before creating questions..."
                )
            ...
            if temp_lang_id is None or temp_cat_id is None:
                logger.error(
                    f"Could not obtain ID for language '{language.name}' or category '{category.name}'. Skipping question '{q_data['text'][:30]}...'"
                )
                continue
    ...
    if session.new or session.dirty:  # Проверяем, есть ли что коммитить
        logger.info(
            f"Committing {len(session.new)} new objects and {len(session.dirty)} dirty objects to the database."
        )
        session.commit()
        ...
        logger.info(
            "No new or modified data to commit for initial population (data likely already exists)."
        )
    logger.info(
        f"Initial data population (hardcoded version) complete. Added {questions_added_count} new questions."
    )
...
def create_and_populate_database():
    ...
    try:
        ...
        logger.exception(
            "CRITICAL: An error occurred during database creation or initial data population."
        )


if __name__ == "__main__":
    logger.info(
        "Executing script to create database tables and populate initial data (hardcoded version)..."
    )
