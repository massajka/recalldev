...
from src.bot.state_machine import get_user_state, UserState
    ...
    async def command(self):
        ...
        with get_session() as session:
            ...
            if state in [UserState.PRACTICE.value, UserState.WAITING_FOR_ANSWER.value]:
                await self.update.message.reply_text(messages.MSG_THANKS_FOR_ANSWER_ANALYZING)
                answer_text = self.update.message.text
                p_res = await practice_flow.process_user_practice_answer(self.context, answer_text)
