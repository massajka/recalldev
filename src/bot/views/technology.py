...
class TechnologyView(View):
    async def command(self):
        ...
        with get_session() as session:
            ...
            if not technologies:
                await self.update.message.reply_text(messages.MSG_NO_AVAILABLE_TECHNOLOGIES)
                return
            keyboard = [
                [InlineKeyboardButton(tech.name, callback_data=f"{callback_data.LANG_SELECT_PREFIX}{tech.id}")]
                for tech in technologies
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await self.update.message.reply_text(messages.MSG_CHOOSE_TECHNOLOGY, reply_markup=reply_markup)
