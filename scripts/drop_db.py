...
def drop_database():
    ...
    if os.path.exists(db_file):
        ...
        try:
            logger.info(
                "Disposing existing engine connections (if any) before deleting DB file..."
            )
            ...
            if (
                hasattr(e, "winerror") and e.winerror == 32
            ...
            logger.exception(
                f"An unexpected error occurred while trying to dispose engine or delete DB file '{db_file}':"
            )
    else:
