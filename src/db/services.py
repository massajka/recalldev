...
from src.db.db import (  # engine нужен для create_all в populate_initial_data
    engine,
    get_session,
)
from src.db.models import SQLModel  # Для SQLModel.metadata.create_all
...
def get_or_create_language(
    ...
    language = session.exec(
        select(ProgrammingLanguage).where(ProgrammingLanguage.slug == slug)
    ).first()
    if not language:
...
def get_language_by_id(
    session: Session, language_id: int
...
def get_category_by_name_and_language(
    session: Session, name: str, language_id: int
) -> Optional[Category]:
    """
    ...
    question_exists = session.exec(
        select(Question).where(
            Question.category_id == category.id, Question.language_id == language_id
        )
    ).first()
    if question_exists:
        return category
...
# --- User Services ---
def create_user(
    session: Session, telegram_id: int, ui_language_code: str = "ru"
) -> User:
...
def set_user_active_language(
    session: Session, user_id: int, language_id: int
) -> Optional[User]:
...
def create_question(
    ...
    if existing_question:
        logger.warning(
            f"Question with text '{text}' for lang_id={language_id}, cat_id={category_id} already exists."
        )
    ...
    logger.info(
        f"Created question: {text[:50]}... for lang_id={language_id}, cat_id={category_id}"
    )
    return question


def get_diagnostic_questions(
    session: Session, language_id: int, category_id: Optional[int] = None
) -> List[Question]:
    statement = (
        select(Question)
        .where(Question.language_id == language_id)
        .where(Question.is_diagnostic == True)
    )
    if category_id:
...
def get_learning_plan_items(
    session: Session, user_progress_id: int
) -> List[UserLearningPlanItem]:
    return session.exec(
        select(UserLearningPlanItem).where(
            UserLearningPlanItem.user_progress_id == user_progress_id
        )
    ).all()
...
def user_has_practice_plan(session: Session, user_progress_id: int) -> bool:
    ...
    return (
        session.exec(
            select(UserLearningPlanItem.id).where(
                UserLearningPlanItem.user_progress_id == user_progress_id
            )
        ).first()
        is not None
...
def add_question_to_learning_plan(
    session: Session,
    ...
    logger.info(
        f"Added question {question_id} to plan for progress {user_progress_id} at index {order_index}"
    )
    return item
...
) -> Optional[UserLearningPlanItem]:
    return session.get(UserLearningPlanItem, item_id)


def get_current_learning_item(
    session: Session, user_progress_id: int
) -> Optional[UserLearningPlanItem]:
    return session.exec(
        select(UserLearningPlanItem)
...
def get_next_pending_learning_item(
    session: Session, user_progress_id: int, current_order_index: Optional[int] = -1
) -> Optional[UserLearningPlanItem]:
    statement = (
        select(UserLearningPlanItem)
        .where(UserLearningPlanItem.user_progress_id == user_progress_id)
    ...
    if current_order_index is not None:
        statement = statement.where(
            UserLearningPlanItem.order_index > current_order_index
        )
...
def update_learning_item_status(
    session: Session, item_id: int, status: str
) -> Optional[UserLearningPlanItem]:
...
def set_current_learning_item(
    session: Session, user_progress_id: int, new_current_item_id: int
) -> Optional[UserLearningPlanItem]:
    ...
    if new_item:
        ...
        logger.info(
            f"Set learning item {new_current_item_id} to 'current' for progress {user_progress_id}"
        )
        return new_item
    return None


def save_diagnostic_scores(
    session: Session, user_progress_id: int, scores: Dict[str, int]
):
    progress = session.get(UserProgress, user_progress_id)
    if progress:
...
def save_diagnostic_answer(
    session: Session, user_progress_id: int, question_id: int, score: int
):
    ...
    if answer:
        ...
        answer = UserDiagnosticAnswer(
            user_progress_id=user_progress_id, question_id=question_id, score=score
        )
        session.add(answer)
        session.commit()
        session.refresh(answer)
...
def get_diagnostic_answers_for_progress(session: Session, user_progress_id: int):
    ...
    return session.exec(
        select(UserDiagnosticAnswer).where(
            UserDiagnosticAnswer.user_progress_id == user_progress_id
        )
    ).all()
...
def save_user_answer(
    ...
    user_id: int,
    question_id: int,
    learning_plan_item_id: int,
    answer_text: str,
    llm_assessment: Optional[str] = None,
    llm_explanation: Optional[str] = None,
    is_correct_by_llm: Optional[bool] = None,
) -> UserAnswer:
    answer = UserAnswer(
        user_id=user_id,
        question_id=question_id,
    ...
    session.refresh(answer)
    logger.info(
        f"Saved answer for user {user_id}, question {question_id}, item {learning_plan_item_id}"
    )
    return answer
...
INITIAL_DATA = {
    ...
    "diagnostic_questions": {
        "python": [
            {
                "category": "Основы языка",
                "text": "Какие основные типы данных в Python вы знаете? Приведите примеры.",
                "notes": "Ожидается перечисление int, float, str, list, tuple, dict, set, bool.",
                "is_diagnostic": True,
            },
            {
                "category": "Структуры данных",
                "text": "Чем отличаются списки (list) от кортежей (tuple) в Python?",
                "notes": "Изменяемость, производительность для некоторых операций, использование.",
                "is_diagnostic": True,
            },
            {
                "category": "ООП (Объектно-ориентированное программирование)",
                "text": "Что такое декоратор в Python и как его создать?",
                "notes": "Функция, принимающая другую функцию и расширяющая её поведение.",
                "is_diagnostic": True,
            },
        ],
        "javascript": [
            ...
            {
                "category": "Асинхронность и параллелизм",
...
def populate_initial_data(session: Session):
    ...
    for lang_data in INITIAL_DATA["languages"]:
        lang = get_or_create_language(
            session, name=lang_data["name"], slug=lang_data["slug"]
        )
    ...
    for lang_slug, questions_data in INITIAL_DATA["diagnostic_questions"].items():
        ...
        if not language:
            logger.warning(
                f"Language slug '{lang_slug}' for diagnostic questions not found in loaded languages. Skipping."
            )
            continue
        ...
        for q_data in questions_data:
            ...
            if not category:
                logger.warning(
                    f"Category '{q_data['category']}' for diagnostic question not found. Skipping question: {q_data['text'][:30]}..."
                )
                continue
